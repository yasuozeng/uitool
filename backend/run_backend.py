"""
uiTool1.0 后端服务启动脚本（生产模式）
不使用 reload，确保 Windows 事件循环策略正确设置
"""
# 导入 sys 模块用于检测操作系统平台
import sys
# 导入 asyncio 异步编程模块
...
# 启动 uvicorn 服务器（生产模式，不使用 reload）
if __name__ == "__main__":
    print("[INFO] 正在启动 uiTool1.0 后端服务（生产模式）...")
    # 启动服务器
    uvicorn.run(
        "app.main:app",  # 应用模块路径
        host="0.0.0.0",  # 监听所有网络接口
        port=8000,  # 监听端口 8000
        reload=False,  # 关闭自动重载（确保事件循环策略稳定）
        log_level="info"  # 日志级别
    )
